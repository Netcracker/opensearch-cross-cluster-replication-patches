...
class TransportInternalStopIndexReplicationAction @Inject constructor (
...
): HandledTransportAction<ActionRequest, AcknowledgedResponse> (InternalStopIndexReplicationAction.NAME, transportService, actionFilters, ::StopIndexReplicationRequest),
    ...
    override fun doExecute(task: Task?, request: ActionRequest?, listener: ActionListener<AcknowledgedResponse>?) {
        launch(Dispatchers.Unconfined + threadPool.coroutineContext()) {
            ...
            try {
                val response = client.suspendExecute(StopIndexReplicationAction.INSTANCE, transformedRequest, true)
